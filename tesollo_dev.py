...
def rate_limit_target(motor_id, desired, prev):
    ...
    if motor_id == 2: max_speed = 65.0  # CMC Flexion speed
...
def clamp_step_to_current(motor_id, desired, current):
    ...
    if motor_id == 2: max_step = 130  # CMC Flexion step
    elif motor_id == 3: max_step = 150
...
def main():
    ...
    try:
        while True:
            ...
            # Resolution control
            if key == ord("+") or key == ord("="): change_resolution(1)
            elif key == ord("-") or key == ord("_"): change_resolution(-1)
            ...
            try:
                ...
                try: gr.set_duty(make_zero_duty())
                except: pass
                reset_duty_state()
